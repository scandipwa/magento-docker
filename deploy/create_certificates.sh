...
if [ -f scandipwa-ca.key ] && [ -f scandipwa-ca.pem ]; then
  ...
  openssl req -x509 -newkey rsa:2048 -out scandipwa-ca.pem -outform PEM -days 1825 -passin pass:${RANDOM_TMP_PASS} -passout pass:${RANDOM_TMP_PASS}
...
if [ ! -f scandipwa-ca.key ] && [ ! -f scandipwa-ca.pem ]; then
  ...
  export OPENSSL_CONF=$CA_CONF_LOCATION
  yes | openssl ca -in tempreq.pem -out server_crt.pem -passin pass:${RANDOM_TMP_PASS}
